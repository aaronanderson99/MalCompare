...
##  PDFDocument
##
#SlayerNEO: adding logger to the class
class PDFDocument(Log):
    ...
    def __init__(self, parser, password='', caching=True, fallback=True):
        ...
        for xref in self.xrefs:
            ...
            # SlayerNEO Patch for PdfDocument: skips ROOT object check to handle malformed PDF files (Only to use with
            # SlayerNEO since the check for the validity of the PDF file is performed on a feature-base).
            #raise PDFSyntaxError('No /Root object! - Is this really a PDF?')
            self.logger.warning("Warning: no Root object has been found for the sample. The analysis will continue, but probably malformed file")
        # SlayerNEO patch to handle malformed files without Root object and a wrong Catalog
        if self.catalog == None:
            self.logger.warning("Warning: no Catalog object has been found for the sample. The analysis will continue, but probably malformed file")
        else:
