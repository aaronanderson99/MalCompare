    ...
    def do_keyword(self, pos, token):
        ...
        if token in (self.KEYWORD_XREF, self.KEYWORD_STARTXREF):
            ...
            try:
                ...
                # SlayerNEO: patching error on wrong literals (might lose some object information, but shouldn't crash anymore)
                if type(objid) != int:
                    pass
                else:
...
class PDFStreamParser(PDFParser):
    ...
    def do_keyword(self, pos, token):
        if token is self.KEYWORD_R:
            ...
            try:
                ((_, objid), (_, genno)) = self.pop(2)
                #SlayerNEO: patch to handle malformed files (probably the X-Ref table wrongly points to that specific object)
                if type(objid) != int:
                    self.logger.warning("The type of object %s is not an integer. Malformed object?", objid)
                else:
                    (objid, genno) = (int(objid), int(genno))
                    obj = PDFObjRef(self.doc, objid, genno)
                    self.push((pos, obj))
