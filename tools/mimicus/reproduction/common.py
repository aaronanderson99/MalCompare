...
import mimicus.tools.manager as manager
...
def attack_gdkde(scenario_name, plot=True):
    # '''
    ...
    # initial_cm = manager.ConfusionMatrix()
    # final_cm = manager.ConfusionMatrix()
    ...
    #     initial_cm.add(fseq[0] > 0, (fseq[0] > 0) == ("mal" in oldf.split("/")[-2]))
    #     final_cm.add(fseq[-1] > 0, (fseq[-1] > 0) == ("mal" in oldf.split("/")[-2]))

    # print("Confusion Matrix using PDFrate prediction of original PDF")
    # initial_cm.printCM()
    # print("\nConfusion Matrix using PDFrate prediction of GD-KDE result")
    # final_cm.printCM()
...
def attack_mimicry(scenario_name, plot=False):
    ...
    # wolf_cm = manager.ConfusionMatrix()
    # mimic_cm = manager.ConfusionMatrix()
    ...
    #     print '{p} : {s}'.format(p=path.basename(wolf_path), s="malicious" if wolf_score > 0 else "benign")
    ...
    #     wolf_cm.add(wolf_score > 0, (wolf_score > 0) == ("mal" in wolf_path.split("/")[-2]))
    #     mimic_cm.add(mimic_score > 0, (mimic_score > 0) == ("mal" in wolf_path.split("/")[-2]))
    ...
    # print("Confusion Matrix using PDFrate prediction of original PDF")
    # wolf_cm.printCM()
    # print("\nConfusion Matrix using PDFrate prediction of mimicry result")
    # mimic_cm.printCM()
